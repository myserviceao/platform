...
public class ServiceTitanClient
{
    ...
    public async Task<string> GetAppointmentsExportAsync(string accessToken, string stTenantId, string? from = null)
    {
        var url = $"{BaseUrl}/jpm/v2/tenant/{stTenantId}/export/appointments";
    ...
    {
        var url = $"{BaseUrl}/jpm/v2/tenant/{stTenantId}/jobs?jobStatus={status}&page={page}&pageSize=50";
        return await GetAsync(accessToken, url);
    }
