...
public class ServiceTitanSyncService
{
    ...
    public async Task<SyncResult> SyncAllAsync(int tenantId)
    {
        ...
        {
            _logger.LogError(ex, "[ST Sync] RecurringServiceEvents failed tenantId={TenantId}", tenantId);
            errors.Add("recurring-service-events: " + ex.Message);
        }
        ...
        _logger.LogInformation("[ST Sync] Complete tenantId={TenantId} AR={AR} OpenJobs={Jobs} OverduePMs={PMs} Errors={Errors}",
            tenantId, snapshot.AccountsReceivable, snapshot.OpenJobCount, snapshot.OverduePmCount, errors.Count);

        return new SyncResult
        {
            Success = true, // partial success is still success — data was saved
            SyncedAt = snapshot.SnapshotTakenAt,
